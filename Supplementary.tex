...
up $\rho\%$ while the epistatic terms make up the remaining $(1-\rho)\%$. Alternatively, the proportion of broad-sense heritability that is explained by additive effects is said to be $\V[\bX_{\mathcal{C}}\bbeta_{\mathcal{C}}] = H^2\rho$, while the proportion of heritability that is detailed by interactions is $H^2(1-\rho)$. Lastly, we introduce population stratification effects by using the top 10 local principal components as the covariates in $\bZ$ and scaling their effects to account for an additional 10\% of the overall phenotypic variance. Overall, analyze 50 simulated datasets per scenario. The barplots show mean power and false discovery rates (FDR) as a function of $\rho$ for \textbf{(a)} $H^2 = 0.6$ and \textbf{(b)} $H^2 = 0.8$, respectively. Standard error bars are also included. QQ-plots from this simulation scheme can be found in Supplementary Figure \ref{MAPITR-Supp-Figure-Simulations-pt2}.}
}
