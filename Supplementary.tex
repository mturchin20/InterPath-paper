...
\caption{\textbf{QQ-Plots of MAPIT-R performance under different pathway definition schemes}. Here, we show QQ-plots from running 50 `null' simulations of the model described in Supplementary Figure \ref{MAPITR-Supp-Figure-Simulations-pt1} with two different pathway definition schemes. Here, `null' means epistatic effect sizes were assigned values of zero instead of being drawn as standard normals, representing a scenario where there is no interactions between pathway SNPs and the rest of the genome. (a) shows a pathway definition scheme where pathway SNPs were chosen from the genome without replacement, meaning there is no SNP overlap between pathways, and (b) shows a pathway definition scheme where pathway SNPs were chosen from the genome with replacement, meaning there can be SNP overlap between pathways. For both plots, the $x$-axis shows the -$\log_{10}$ expected $p$-value and the $y$-axis shows the -$\log_{10}$ observed $p$-value. The blue 
Barplots simulation results, including from a range of genetic architectures, are shown in Supplementary Figure \ref{MAPITR-Supp-Figure-Simulations-pt1}.
