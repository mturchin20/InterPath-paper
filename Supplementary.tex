...
\centering
%\hspace*{-1cm}
\includegraphics[width = \textwidth]{Images/Supp/SupplementaryFigure1.png}
...
\label{MAPITR-Supp-Figure-PCAPlots}
\end{figure}
\clearpage
...
\begin{figure}[htbp]
\centering
\includegraphics[scale=.6]{Images/Supp/SupplementaryFigure3.png}
\caption{\textcolor{red}{\textbf{Barplots of MAPIT-R performance under different simulated genetic architectures}. Here, we show results from running simulations of MAPIT-R under various genetic architectures. We run a simulation scheme similar to that used in Crawford et al. \cite{Crawford2017a}. For all simulations, 3,000 random individuals from the British subgroup and 10,000 random SNPs from chromosome 1 were used. We simulated continuous phenotypes using the following model: $\by =  \bX\bbeta + \bZ\bu + \bW\balpha + \bepsilon$, where $\bX$ is a genotype matrix, $\bZ$ is a covariate matrix representing population structure, $\bW$ is a design matrix that contains SNP-by-SNP interaction designations, and $\bu$, $\bbeta$, and $\balpha$ are all effect sizes that are drawn from standard normal distributions, or $\N(\bm{0},\bI)$. The random noise term, $\bepsilon$, is also drawn as a standard normal variable. Epistatic interactions are introduced by first creating pathways and then designating interactions between these pathways and the rest of the genome. Specifically, we create ten pathways per simulation by randomly selecting 50 `pathway SNPs' with replacement per pathway, and we then create interactions by selecting 200 `partner SNPs' for each pathway SNP from the remaining set of SNPs. The designations of which pathway SNPs interact with which partner SNPs are coded in $\bW$. In total, 100,000 interactions are created per simulation (10 pathways * 50 pathway SNPs * 200 partner SNPs). After selecting epistatic SNPs, we then select a random 50\% of the remaining SNPs to represent background additive effects. Effects sizes are drawn as described above for both epistatic and additive SNPs. To properly control phenotype variance, we first scale overall heritability ($H^2$) as .6 or .8. We then scale the additive heritability, represented by the background additive SNPs, as $H^2*(\rho)$ and scale the epistatic heritability, represented by all the pathway-and-partner interacting SNPs, as $H^2*(1-\rho)$. The parameter $\rho$ controls the ratio between additive and epistatic heritability and ranges from from .6 to .9. Lastly, we introduce population structure effects into the simulation by using the top 10 local principal components from the selected individuals as the covariates in $\bZ$ and scaling their effects to account for an additional 10\% of the overall phenotype variance. Given this model and design scheme we run and analyze 50 simulated datasets. The barplots show power and false discovery rates (FDR) from these simulations. $H^2$ of .6 is shown on the left and $H^2$ is shown on the right. The $x$-axis shows the different values of $\rho$ and the $y$-axis shows power. Colors correspond to different value of $\rho$ and the legends show each corresponding FDR. Error bars are included for each bar. QQ-plots of simulation results, including from two different pathway design schemes, are shown in Supplementary Figure \ref{MAPITR-Supp-Figure-Simulations-pt2}.}
}
\label{MAPITR-Supp-Figure-Simulations-pt1}
\end{figure}
%\end{landscape}
\clearpage
...
\begin{figure}[htbp]
\centering
\subfigure[No Pathway Overlap]{\includegraphics[width=.45\textwidth]{Images/Supp/SupplementaryFigure4a.png}}
\subfigure[Pathway Overlap]{\includegraphics[width=.45\textwidth]{Images/Supp/SupplementaryFigure4b.png}}
\caption{\textcolor{red}{\textbf{QQ-Plots of MAPIT-R performance under different pathway definition schemes}. Here, we show QQ-plots from running 50 `null' simulations of the model described in Supplementary Figure \ref{MAPITR-Supp-Figure-Simulations-pt1} with two different pathway definition schemes. `Null' here means epistatic effect sizes were assigned values of zero instead of being drawn as standard normal variables; as a result, this setup represents a scenario where there is no interactions between pathway SNPs and the rest of the genome, hence being a test of the null hypothesis of no epistatic interactions. (a) shows a pathway definition scheme where pathway SNPs were chosen from the genome without replacement, meaning there is no SNP overlap between pathways. (b) shows a pathway definition scheme where pathway SNPs were chosen from the genome with replacement, meaning there can be SNP overlap between pathways. For both plots, the $x$-axis shows the -$\log_{10}$ expected $p$-value and the $y$-axis shows the -$\log_{10}$ observed $p$-value. Blue points represent the 500 total (10 pathways * 50 simulations) pathways simulated and tested. Grey windows represent 95\% confidence intervals. Barplots of simulation results, including from a wider range of simulated genetic architectures, are shown in Supplementary Figure \ref{MAPITR-Supp-Figure-Simulations-pt1}}.
}
\label{MAPITR-Supp-Figure-Simulations-pt2}
\end{figure}
%\end{landscape}
\clearpage
...
\begin{figure}[htbp]
\centering
\includegraphics[scale=.2]{Images/Supp/SupplementaryFigure10.png}
\caption{\textbf{Heatmaps depicting the overlap of MAPIT-R significant REACTOME pathways for (a) standing height and (b) body mass index (BMI) per British replicate subgroup in the UK Biobank}. Here, the random subgroups were created by subsampling either $N =$ 4,000 or 10,000 individuals from the full British cohort. In the legend, replicates are marked as numbers after the root tag name (e.g. \texttt{Brit.Ran4k.4} denotes the fourth replicate in the $N =$ 4,000 subsampling scheme). Genome-wide significance was determined by using Bonferroni-corrected $p$-value thresholds based on the number of pathways tested in each database-phenotype-subgroup combination (see Supplementary Table \ref{MAPITR-Supp-Table-UKBPopStats}). The diagonal shows the total number of genome-wide significant pathways per subgroup. We do not observe much overlap between enriched pathways between the replicate subgroups --- even though they all consist of individuals of the same ancestry. \textcolor{red}{See Supplementary Figures \ref{MAPITR-Supp-Figure-BritReps-Barplots}-\ref{MAPITR-Supp-Figure-BritReps-Heatplots-KEGG},\ref{MAPITR-Supp-Figure-BritReps-Perms-QQPlots}-\ref{MAPITR-Supp-Figure-BritReps-Perms-pValHists-pt2} and Supplementary Tables \ref{MAPITR-Supp-Table-BritReps-FDRs-pt1}-\ref{MAPITR-Supp-Table-BritReps-FDRs-pt2} for other analyses using the 4,000 and 10,000 British replicate subgroups.}}
\label{MAPITR-Supp-Figure-BritReps-Heatplots-REACTOME}
\end{figure}
\clearpage
\end{landscape}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\textwidth]{Images/Supp/SupplementaryFigure11.png}
\caption{\textbf{QQ-Plots of MAPIT-R $\bm{p}$-values using KEGG and REACTOME pathway annotations and randomly permuted phenotypes in British replicate subgroups from the UK Biobank.} Here, we run MAPIT-R after conducting a single random permutation of either height or BMI measurements. Note that traits were permuted within each population subgroup ten different times. The $x$-axis shows the -$\log_{10}$ transformed expected $p$-values, while the $y$-axis shows the -$\log_{10}$ observed $p$-values. The random subgroups were created by subsampling either $N =$ 4,000 or 10,000 individuals from the full British cohort. In the legend, replicates are marked as numbers after the root tag name (e.g. \texttt{Brit.Ran4k.4} denotes the fourth replicate in the $N =$ 4,000 subsampling scheme). The dotted red line is the Bonferroni-corrected $p$-value threshold based on the number of pathways tested per database-phenotype combination (Supplementary Table \ref{MAPITR-Supp-Table-UKBPopStats}). Overall, we find that MAPIT-R continues to exhibit well-calibrated behavior under the null hypothesis and properly controls for type 1 error rate. \textcolor{red}{See Supplementary Figures \ref{MAPITR-Supp-Figure-BritReps-Barplots}-\ref{MAPITR-Supp-Figure-BritReps-Heatplots-REACTOME},\ref{MAPITR-Supp-Figure-BritReps-Perms-pValHists-pt1}-\ref{MAPITR-Supp-Figure-BritReps-Perms-pValHists-pt2} and Supplementary Tables \ref{MAPITR-Supp-Table-BritReps-FDRs-pt1}-\ref{MAPITR-Supp-Table-BritReps-FDRs-pt2} for other analyses using the 4,000 and 10,000 British replicate subgroups.}}
\label{MAPITR-Supp-Figure-BritReps-Perms-QQPlots}
\end{figure}
\clearpage
...
\centering
%\vspace*{-1cm}
\includegraphics[width=\textwidth]{Images/Supp/SupplementaryFigure13.png}
\caption{\textbf{Histograms of MAPIT-R $\bm{p}$-values using KEGG and REACTOME pathway annotations and randomly permuted phenotypes in each \textcolor{red}{British 10,000 replicate} subgroup in the UK Biobank.} Here, we run MAPIT-R after conducting a single random permutation of either height or BMI measurements. Note that traits were independently permuted within each population subgroup ten different times. The random subgroups were created by subsampling $N =$ 10,000 individuals from the full British cohort. In the row labels, replicates are marked as numbers after the root tag name (e.g. \texttt{Brit.Ran10k.4} denotes the fourth replicate in the $N =$ 10,000 subsampling scheme). The dotted red line corresponds to a uniform distribution of $p$-values. Overall, we find that MAPIT-R continues to exhibit well-calibrated behavior under the null hypothesis. \textcolor{red}{See Supplementary Figures \ref{MAPITR-Supp-Figure-BritReps-Barplots}-\ref{MAPITR-Supp-Figure-BritReps-Perms-pValHists-pt1} and Supplementary Tables \ref{MAPITR-Supp-Table-BritReps-FDRs-pt1}-\ref{MAPITR-Supp-Table-BritReps-FDRs-pt2} for other analyses using the 4,000 and 10,000 British replicate subgroups.}}
\label{MAPITR-Supp-Figure-BritReps-Perms-pValHists-pt2}
\end{figure}
\clearpage
%\setlength{\footskip}{1cm}
...
\centering
\vspace*{-2.2cm}
\includegraphics[angle=270,scale=0.179]{Images/Supp/SupplementaryFigure15.png}
...
\label{MAPITR-Supp-Figure-pValsVsNumSNPs-Perms}
\end{figure}
%\setlength{\footskip}{1cm}
\end{landscape}
...
\centering
\vspace*{-.5cm}
\subfigure[$\Delta$ in MAPIT-R -$\log_{10}$ $p$-value per SNP]{\includegraphics[width=.9\textwidth]{Images/Supp/SupplementaryFigure16.png}}
...
\centering
\vspace*{-.5cm}
\subfigure[$\Delta$ in MAPIT-R -$\log_{10}$ $p$-value per SNP]{\includegraphics[width=\textwidth]{Images/Supp/SupplementaryFigure17.png}}
\subfigure[\textcolor{red}{Number of SNPs in each Proteasome Gene Family and REACTOME Pathway}]{
\setlength{\extrarowheight}{3pt}
 \hspace*{-1cm}
 \begin{tabular}{|cc|ccc|}
...
\label{MAPITR-Supp-Figure-Proteasome-Heatplots-BritRan4000}
\end{figure}
...
\centering
\vspace*{-.5cm}
\subfigure[$\Delta$ in MAPIT-R -$\log_{10}$ $p$-value per SNP]{\includegraphics[width=\textwidth]{Images/Supp/SupplementaryFigure18.png}}
\subfigure[\textcolor{red}{Number of SNPs in each Proteasome Gene Family and REACTOME Pathway}]{
\setlength{\extrarowheight}{3pt}
 \hspace*{-1cm}
 \begin{tabular}{|cc|ccc|}
...
\label{MAPITR-Supp-Figure-Proteasome-Heatplots-Caribbean}
\end{figure}
...
\centering
\vspace*{-.5cm}
\subfigure[$\Delta$ in MAPIT-R -$\log_{10}$ $p$-value per SNP]{\includegraphics[width=\textwidth]{Images/Supp/SupplementaryFigure19.png}}
\subfigure[\textcolor{red}{Number of SNPs in each Proteasome Gene Family and REACTOME Pathway}]{
\setlength{\extrarowheight}{3pt}
 \hspace*{-1cm}
 \begin{tabular}{|cc|ccc|}
...
\label{MAPITR-Supp-Figure-Proteasome-Heatplots-Chinese}
\end{figure}
...
\centering
\vspace*{-.5cm}
\subfigure[$\Delta$ in MAPIT-R -$\log_{10}$ $p$-value per SNP]{\includegraphics[width=\textwidth]{Images/Supp/SupplementaryFigure20.png}}
\subfigure[\textcolor{red}{Number of SNPs in each Proteasome Gene Family and REACTOME Pathway}]{
\setlength{\extrarowheight}{3pt}
 \hspace*{-1cm}
 \begin{tabular}{|cc|ccc|}
...
\label{MAPITR-Supp-Figure-Proteasome-Heatplots-Indian}
\end{figure}
...
\centering
\vspace*{-.5cm}
\subfigure[$\Delta$ in MAPIT-R -$\log_{10}$ $p$-value per SNP]{\includegraphics[width=\textwidth]{Images/Supp/SupplementaryFigure21.png}}
\subfigure[\textcolor{red}{Number of SNPs in each Proteasome Gene Family and REACTOME Pathway}]{
\setlength{\extrarowheight}{3pt}
 \hspace*{-1cm}
 \begin{tabular}{|cc|ccc|}
...
\label{MAPITR-Supp-Figure-Proteasome-Heatplots-Pakistani}
\end{figure}
...
\centering
\hspace*{-2cm}
\includegraphics[angle=270,scale=.25]{Images/Supp/SupplementaryFigure22.png}
...
\label{MAPITR-Supp-Figure-PhenoComps-AllPops}
\end{figure}
\end{landscape}
\setlength{\footskip}{1cm}
...
%\vspace*{-1.75cm}
%\hspace*{-1cm}
\includegraphics[width=0.9\textwidth]{Images/Supp/SupplementaryFigure23.png}
...
\label{MAPITR-Supp-Figure-Hypergeometric-SizeThresholds}
\end{figure}
\clearpage
%\setlength{\footskip}{1cm}
...
\label{MAPITR-Supp-Table-AllPops-FDRs}
\end{table}
...
[2pt]\hline
 ...
 \hline
\end{tabular}
\caption{\textbf{Genes that are annotated for multiple enriched pathways in both standing height and body mass index (BMI) within the African subgroup from the UK Biobank.} Here, we compare genes that are present across multiple of the pathways highlighted in blue in Figure \ref{MAPITR-Main-Figure-PhenoComps-African}; these are specific pathways that have more significant MAPIT-R $p$-values in BMI than in height. Genome-wide significance was determined by using Bonferroni-corrected $p$-value thresholds based on the number of pathways tested in each database-phenotype-subgroup combination with significance value $\alpha = 0.05$ (see Supplementary Table \ref{MAPITR-Supp-Table-UKBPopStats}). The second column records the number of enriched pathways that each gene belongs to, according to the KEGG database.}
\label{MAPITR-Supp-Table-PhenoComps-African-GeneCounts}
\end{table}
...
\multirow{10.5}{*}{\textbf{REACTOME (BMI)}} & British (4k, Rep.~1) & 6490 & 7.704$\times 10^{-5}$ & 1 & 0.015 & 4 & 0.062 & 52 & 0.801 \\
  ...
  & British (10k, Rep.~4) & 6690 & 7.474$\times 10^{-5}$ & 0 & 0.000 & 5 & 0.075 & 57 & 0.852 \\
    \hline
\end{tabular}
\caption{\textbf{MAPIT-R false discovery rates at different significance thresholds using REACTOME pathway annotations and permuted phenotypes across the British replicate subgroups in the UK Biobank.} Here, we run MAPIT-R after conducting a single random permutation of either height or body mass index (BMI) measurements. Note that traits were independently permuted within each population subgroup ten different times. The random subgroups were created by subsampling either $N$ = 4,000 or 10,000 individuals from the full British cohort. Genome-wide significance at a Bonferroni-corrected $p$-value threshold was based on the number of pathways tested in each database-phenotype-subgroup combination with significance value set to $\alpha = 0.05$. The second column lists the total number of pathways that were tested across each of the ten phenotype permutations. \# FPs denotes the number of false positives at that threshold and FDR reports the corresponding false discovery rate (listed as percentages). The remaining columns list the same information but at significance levels $\alpha = 0.001$ and 0.01, respectively. \textcolor{red}{See Supplementary Figures \ref{MAPITR-Supp-Figure-BritReps-Barplots}-\ref{MAPITR-Supp-Figure-BritReps-Perms-pValHists-pt2} and Supplementary Table \ref{MAPITR-Supp-Table-BritReps-FDRs-pt1} for other analyses using the 4,000 and 10,000 British replicate subgroups.}}
\label{MAPITR-Supp-Table-BritReps-FDRs-pt2}
\end{table}
...
\label{MAPITR-Supp-Table-TopGeneCounts}
\end{table}
...
\label{MAPITR-Supp-Table-TopGeneCounts-SizeRestricted}
\end{table}
