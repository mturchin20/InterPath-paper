...
We expanded the previous MAPIT model into a new framework that goes from looking for marginal epistasis with a single variant to looking for marginal epistasis with an entire biological pathway; we call this approach InterPath, for \underline{Inter}action and \underline{Path}way analysis. The main change we make to the marginal epistasis model is to go from a single SNP of interest \textbf{k} to a collection of SNPs of interest \textbf{R}. For a full description of this new model please see Online Methods, In the fully additive form of the model, this results in the following:
\begin{equation}
    \textbf{y} = \mu + \sum_{r \in R} \textbf{x}_r\beta_r + \sum_{l \notin R} \textbf{x}_l\beta_l + \sum_{r \in R}\sum_{l \notin R} (\textbf{x}_r \circ \textbf{x}_l)\alpha_{rl} + \boldsymbol{\epsilon}, \quad \boldsymbol{\epsilon} \sim \mathcal{MVN}(\textbf{0}, \tau^{2}\textbf{I})  
\end{equation}
