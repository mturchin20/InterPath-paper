...
To begin investigating epistasis in non-European ancestries, we first extracted multiple non-European population subsets from the UK BioBank (UKB) dataset, including  African, Caribbean, Chinese, Indian, and Pakistani subpopulations (Supplementary Table \ref{InterPath-Supp-Table-UKBPopStats}). For comparative purposes, we also extracted a British subset, but to match the sample sizes of the non-European subsets we took a random subsample of 4,000 British individuals. Across all these population subsets, we conducted typical quality control (QC) procedures (see Online Methods) and focused our analyses on the complex traits of height and body mass index (BMI). 
...
The second of these two problems with our pathway distributions may be more easy to immediately deal with. If we are concerned about this relationship between pathways being larger and producing inflated hypergeometric $p$-values, then we possible approach may be to restrict the size of the pathways we analyze. Instead of looking at enrichment among all our pathways, we can instead look at enrichment only among pathways that have under a certain limit of SNPs. And indeed, by restricting the number of SNPs present in a pathway to 1,000 or less, we see a major drop off in the number of significant pathways remaining (29 to 2 for KEGG height, 47 to 11 for KEGG BMI, 24 to 0 for REACTOME height, and 65 to 26 for REACTOME BMI); this, by extension, also produces a large drop in the genes that are present across these pathways, and their associated hypergeometric $p$-values as well (Supplementary Figure \ref{InterPath-Supp-Figure-Hypergeometric-RestrictedComps-African-BMI} and Supplementary Table \ref{InterPath-Supp-Table-Hypergeometric-RestrictedComps-African-BMI-TopExamples}). Interestingly however, we find one gene family that has had the opposite result -- multiple proteasome subunit genes (\textit{PSMA}*, \textit{PSMB}*, \textit{PSMC}*, \textit{PSMD}*, and \textit{PSME*}) now produce more significant hypergeometric $p$-values upon restricting pathways to smaller SNP counts (Supplementary Table \ref{InterPath-Supp-Tables-AllPops-TopGeneCount-HypergeometricTests}). 
