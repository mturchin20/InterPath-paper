...
The second of these two problems with our pathway distributions may be more easy to immediately deal with. If we are concerned about this relationship between pathways being larger and producing inflated hypergeometric $p$-values, then we possible approach may be to restrict the size of the pathways we analyze. Instead of looking at enrichment among all our pathways, we can instead look at enrichment only among pathways that have under a certain limit of SNPs. And indeed, by restricting the number of SNPs present in a pathway to 1,000 or less, we see a major drop off in the genes that are present across these pathways as well as their associated hypergeometric $p$-values (Supplementary Table \ref{}). Interestingly however, we find one gene family that has had the opposite result -- multiple proteasome subunit genes (\textit{PSMA*}, \textit{PSMB*}, \textit{PSMC*}, \textit{PSMD*}, and \textit{PSME*}) now produce more significant hypergeometric $p$-values upon restricting pathways to smaller SNP counts (Supplementary Table \ref{} b or something?). The proteasome is one half of the ubiquitin-proteasome system (UPS), a process that degrades proteins via proteolysis. The UPS is a critical component of how cells modulate protein concentrations, and is often associated with the removal of unneeded or damaged proteins. Recent work has also begun to suggest that regulation of the proteosome itself may be an important component of proper cell homeostasis and involve multiple interacting parnters \citep{}. The genes we find highlighted in our analysis represent multiple subunits of the 26S proteasome, the complex variant that is most often found within the cytosol of mammalian cells \citep{}. Therefore these subunits may be particularly involved with the actual interactions of other proteins, of which there could be many. It may not be surprinsg  





Another trend across all of these pathways is that they often contain many SNPs. In fact, within the African subset, we observe a very strong relationship between number of SNPs included in a pathway and the $-log10$ MAPIT-R $p$-value for both pathway databases and phenotypes (Figure \ref{} \& Supplementary Figure \ref{}). This matches our motivation and hypothesis that including more SNPs might allow us to detect greater signals for epistasis, much like what is observed for additive effects. However, this provides
One important point to consider when evaluating these biological themes is whe

We do see a significant relationship between number of SNPs in a pathway and that pathway's MAPIT-R $p$-value (Supplementary Figure \ref{InterPath-Supp-Figure-pValsVsNumSNPs}), however the African subset has similar pathway SNP counts amongst its top pathways as compared to the other population subsets.

which genes are driving these pathway signals?
