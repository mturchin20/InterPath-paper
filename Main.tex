...
Therefore, to investigate this possible relationship between subgroup PVE on PC1 and MAPIT-R results, we compared the proportion of SNPs that are strongly loaded on PC1 within a given pathway against that pathway's MAPIT-R $p$-value. If the cryptic structure represented by PC1 is driving the African MAPIT-R results, then we might expect pathways with more SNPs that are strongly loaded on PC1 to have more significant MAPIT-R $p$-values as well.  something something something. However, across both phenotypes in both pathway databases in the African subgroup, we find no significant relationship; and looking across the other database + phenotype + subgroup combinations, we find no significantly positive relationships either (Supplementary Figure \ref{InterPath-Supp-Figure-PC1Loading-AllPaths}). 
