...
\subsection{MAPIT-R Model}\label{InterPath-Results-MAPITRModel}
...
\subsection{Many Pathways Identified As Genome-Wide Significant for Marginal Epistatic Interactions in the African Subgroup}\label{InterPath-Results-PathwayEpistasis}

Running MAPIT-R on our UKB subgroups across height and BMI, we find 245 pathways that are genome-wide significant for containing marginal epistatic interactions with the rest of the genome (Figure \ref{InterPath-Main-Figure-Barplots-KEGG}, Supplementary Figure \ref{InterPath-Supp-Figure-Barplots-REACTOME}, and Supplementary Tables \ref{InterPath-Supp-Table-TopPathways-KEGG-Height-a}\textcolor{blue}{-d}; $p$-value significance thresholds were determined using Bonferroni multiple testing correction based on the number of pathways tested per analysis). We find similar numbers of significant pathways between the KEGG and REACTOME databases\red{: 130 and 115, respectively. SR - overlap? Venn diagram? or doesn't matter?}. And in both databases, we find that BMI contains more significant pathways than height (155 vs. 90 - \red{SR: this is summed across both databases for each trait? I'm confused.}). Interestingly, the vast majority of our significant pathways are identified in the African subgroup (165/245). This skew towards the African subgroup is found in each pathway database as well as in each phenotype.\red{SR: in Results, I strongly prefer avoiding adverbs/what I call ``editorializing'' or interpreting results - I think that is for the discussion. Is should just be a walk-through of figures with a sense of what results we want the reader to focus on. The Discussion is where we explain why those are noteworthy.}
...
Expanding beyond the results from our African subgroup, we see 80 other pathways that are significant across the other subgroups. Perhaps unsurprisingly given the abundance of results from the African subgroup, many of these other results overlap with the set of significant pathways from the African subgroup (Figure \ref{InterPath-Main-Figure-Heatplots-KEGG} \& Supplementary Figure \ref{InterPath-Supp-Figure-Heatplots-REACTOME}). For example, in the KEGG Height results the Caribbean subgroup overlaps with the African subgroup for 6 of its 7 pathways, and the Chinese subgroup overlaps with the African subgroup for 7 of its 8 pathways; interestingly, however, there is no overlap between the Chinese and Caribbean subgroups.\red{SR: to my earlier point, not sure why this is interesting - either way, should be more of a discussion point.  But here particularly I don't see why I would expect overlap between these two subgroups?}
...
Looking into the specific pathways in each of these overlaps, we find pathways moreso related to the cellular signaling and heart condition patterns from before in the overlap between the African and Caribbean subgroups; additionally these overlaps appear to be driven by multiple kinases (eg MAPK1, ROCK1, PRKCB, PAK1) and calcium channel proteins (eg CACNA1S, CACNA1D) (Supplementary Tables \ref{InterPath-Supp-Tables-MAPITR-TopPathway-Overlap} \& \ref{InterPath-Supp-Tables-MAPITR-TopPathway-GeneCounts-Overlap}). The overlap between the African and Chinese subgroups however moreso contain pathways related to the immune system and appear to be driven primarily by multiple HLA loci (eg HLA-DRA, HLA-DRB1, HLA-A, HLA-B) (Supplementary Tables \ref{InterPath-Supp-Tables-MAPITR-TopPathway-Overlap} \& \ref{InterPath-Supp-Tables-MAPITR-TopPathway-GeneCounts-Overlap}). These differences between ancestries may reflect heterogeneous power to detect epistatic signals. It is interesting to see the continual overlap between non-African subgroups and the African subgroup, but less so between the non-African subgroups. One possibility we may be witnessing is that pathways with the largest signals for marginal epistasis may be widely important biologically, hence seeing themes such as cellular signaling and the immune system. More population-specific signals may require greater sample sizes to identify. 
...
To test whetehr any genes were significantly enriched across our top pathways, we used a hypergeometric test for enrichment to compare the number of times a gene appears among our significant pathways against the number of times that gene appears in the full distribution of pathways in either the KEGG or REACTOME databases. 
...
To begin investigating epistasis in non-European ancestries\footnote{\red{SR: I still don't understand starting off the results like this - is the point that no one has looked at epistasis in non-Europeans? I would have thought our motivation was looking at epistasis at the pathway level and in multiple ethnicities. To me, ``we looked outside Europe'' is not a good motivation.}}, we first extracted multiple non-European population subgroups from the UK BioBank (UKB) dataset, including\footnote{\red{SR: are there others? including makes it sound like there are more than listed in this sentence.}} individuals of African, Caribbean, Chinese, Indian, and Pakistani self-identified ancestry (Supplementary Figure \ref{InterPath-Supp-Figure-UKB-subgroups-PCAPlot} and Supplementary Table \ref{InterPath-Supp-Table-UKBPopStats}) - \red{for this table, make sure to cite materials and methods in the caption after you mention quality control procedures; also, do those describe why the pathway numbers change? Is it due to SNPs in pathways changing with QC procedures? I think that needs to be clarified in the caption.}. For comparative purposes, we also extracted a British subgroup; but to make it on a matched scale as the non-European subgroup sample sizes, we used a random subsample of 4,000 British individuals.\footnote{\red{SR: ``matched scale'' is vague; overall I find this sentence too wordy. I would stick with ``For comparative purposes'' and then cut to we used a random subsample. Why just one subsample? That seems odd. Also, in the previous sentence you should include sample sizes for the other groups in parentheses after each name.}} Across all these population subgroups, we conducted quality control (QC) procedures (see Materials and Methods) and focused our analyses on the complex traits of height and body mass index (BMI)\footnote{\red{SR: I think the second part of this sentence -- the traits we focus on, should be motivated in the intro.}}. 
