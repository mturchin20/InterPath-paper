...
\begin{table} [t!]
  ...
  Results from gene-based enrichment tests in MAPIT-R identified pathways with at most 1000 SNPs (restricted analysis)}. Attached Excel sheets contain lists
  
  
  The table shows the top examples of genes that became more significant in the hypergeometric enrichment analyses after the size restriction step in both pathway databases. Results are only shown for BMI since the majority of genome-wide significant pathways identified in height (Supplementary Figure \ref{InterPath-Supp-Figure-Barplots-REACTOME}) did not contain at least 1000 SNPs and were therefore excluded from the restricted analysis. 
  
  
  The first column lists the pathway database, the second column lists the genes, the third column lists the hypergeometric enrichment $p$-value from the original analysis (cite figures and tables appropriately), the fourth column lists the hypergeometric enrichment $p$-value when restricting analysis to pathways with at most 1000 SNPs, and the fifth column lists the difference between the -$\log_{10}$ $p$-values from the third and fourth column. The genes displayed are those with the ten largest differences in $p$-values in these two hypergeometric tests, sorted in descending order by the fifth column.
  
  For a plot of results from every gene analyzed in both databases, see Supplementary Figure \ref{InterPath-Supp-Figure-Hypergeometric-RestrictedComps-African-BMI}). 
  
  Note that to preserve space and not list every \emph{PSM} gene family locus a single in the REACTOME results, a representative entry in the form of \emph{PSM*} is used instead.
  
  \textbf{Gene counts across size-restricted MAPIT-R genome-wide significant pathways, per subgroup}. Attached Excel sheets contain lists of genes that appear multiple times across the MAPIT-R genome-wide significant pathways that contain less than or equal to 1,000 SNPs (`size-restricted') in each UKB subgroup for the following pathway database + phenotype combinations: (a) KEGG+Height, (b) KEGG+BMI, (c) REACTOME+Height, and (d) REACTOME+BMI.
  
  Genome-wide significance was determined by using a Bonferroni-corrected $p$-value threshold of .05 divided by the number of pathways tested (Supplementary Table \ref{InterPath-Supp-Table-UKBPopStats}).}
...
\label{InterPath-Supp-Table-AllPops-TopGeneCounts-KEGG-Height-a}
\end{table}
\clearpage
\addtocounter{table}{-1}
...
\label{InterPath-Supp-Table-AllPops-TopGeneCounts-KEGG-BMI-a}
\end{table}
\clearpage
\addtocounter{table}{-1}
\addtocounter{CharNumber2}{1}
...
\label{InterPath-Supp-Table-AllPops-TopGeneCounts-REACTOME-Height-a}
\end{table}
\clearpage
\addtocounter{table}{-1}
\addtocounter{CharNumber2}{1}
...
\label{InterPath-Supp-Table-AllPops-TopGeneCounts-REACTOME-BMI-a}
\end{table}
\clearpage
\addtocounter{table}{-1}
\addtocounter{CharNumber2}{1}
