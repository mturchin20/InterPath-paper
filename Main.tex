...
Therefore, to investigate this possible relationship between subgroup PVE on PC1 and MAPIT-R results, we compared the proportion of SNPs that are strongly loaded on PC1 within a given pathway against that pathway's MAPIT-R $p$-value. something something soemthing. However, across both phenotypes in both pathway databases in the African subgroup, we find no significant relationship, nor do we find any significantly positive relationships in any other ancestry subgroup as well (Supplementary Figure \ref{InterPath-Supp-Figure-PC1Loading-AllPaths}). 
