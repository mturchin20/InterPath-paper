...
One advantage region-based tests offer is the ability to further refine statistically significant signals, whether in GWA or marginal epistatic scenarios. If starting on a pathway-level in an analysis, using a region-based test can allow you to test smaller segments of a pathway to further identify where a statistical signal is localized. We were interested then in identifying whether we could use MAPIT-R to refine any of the significant pathway-level marginal epistatic signals we have found in our UKB subgroups.
...
For our purposes of investigating marginal epistasis, the proteasome offers a unique opportunity to not only further refine the original pathway-level signal, but also to further show the utility of group-based approaches. \textcolor{blue}{from: refining MAPIT-R signals w/ proteasome section}
...
\caption[TBD]{\textbf{Global PCA Plots of the UK BioBank subgroups}. The figure shows the top eight global principal components (PCs) of our UKB subgroup dataset plotted against one another: a) PC1 vs. PC2, b) PC3 vs. PC4, c) PC5 vs. PC6, d) PC7 vs. PC8. PCA was conducted using FlashPCA \citep{Abraham2017} and a post-QC, pruned SNP set. Here `Global' PCs refers to PCs calculated from running PCA on the full dataset of all subgroups together jointly. This is in contrast to `local' PCs, which are calculated by running PCA on each subgroup separately. As a result of this difference, global PCs were created similar to the descriptions in `Population subgroups' of Methods and `Population Subgroup Quality Control' of Supplementary Note, but in lieu of running FlashPCA on each subgroup separately, FlashPCA was run on the full dataset together jointly. Global PCs were only used for these plots, and local PCs were used for quality control and as covariates.}
...
\caption[TBD]{\textbf{Overlap of genome-wide significant MAPIT-R pathways between British replicate subgroups: KEGG}. The heatplots show the numbers of genome-wide significant MAPIT-R pathways that overlap between each British replicate subgroup for height and BMI in the KEGG database. The diagonal shows the total number of genome-wide significant pathways per population subgroup. We do not observe many genome-wide significant pathways or pathways that overlap between replicate subgroups.}
...
\begin{tabular}{ccccc}
  ...
  & ATR & 8.004E-03 & 1.261E-03 & 0.8.02 \\
   \hline
\end{tabular}
\caption[TBD]{\textbf{Results from gene-based enrichment tests in MAPIT-R identified pathways with at most 1000 SNPs (restricted analysis)}. The table shows the top examples of genes that became more significant in the hypergeometric enrichment analyses after the size restriction step in both pathway databases. Results are only shown for BMI since the majority of genome-wide significant pathways identified in height (Supplementary Figure \ref{InterPath-Supp-Figure-Barplots-REACTOME}) did not contain at least 1000 SNPs and were therefore excluded from the restricted analysis. The first column lists the pathway database, the second column lists the genes, the third column lists the hypergeometric enrichment $p$-value from the original analysis (cite figures and tables appropriately), the fourth column lists the hypergeometric enrichment $p$-value when restricting analysis to pathways with at most 1000 SNPs, and the fifth column lists the difference between the -$\log_{10}$ $p$-values from the third and fourth column. The genes displayed are those with the ten largest differences in $p$-values in these two hypergeometric tests, sorted in descending order by the fifth column. For a plot of results from every gene analyzed in both databases, see Supplementary Figure \ref{InterPath-Supp-Figure-Hypergeometric-RestrictedComps-African-BMI}). Note that to preserve space and not list every \emph{PSM} gene family locus a single in the REACTOME results, a representative entry in the form of \emph{PSM*} is used instead.}
