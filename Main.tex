...
Supplementary Table \ref{} shows the various genes present across our top pathways at different levels of counts, and Supplementary Figure \ref{} shows the QQ-plot of our hypergeoemtric enrichment $p$-values across these genes. What immediately stands out is the abundance of `significant' $p$-values we find. Even using the conservative Bonferroni cutoff of XXX (.05 / XX genes), we would identify XXX genes as being significantly enriched. However, one major assumption undelrying this test is that each pathway is independent from one another, and we know this to not be the case. In fact this assumption is being violated in two ways in regards to the distribution of pathways we are analyzing. 
...
The second of these two problems with our pathway distributions may be more easy to immediately deal with. If we are concerned about this relationship between pathways being larger and producing inflated hypergeometric $p$-values, then we possible approach may be to restrict the size of the pathways we analyze. Instead of looking at enrichment among all our pathways, we can instead look at enrichment only among pathways that have under a certain number of SNPs. 

it may also be possible that these are genes which are just pre there being a greater likelihood that the pathways we identify as signfiicant  between pathways having more SNPs and genes and tsince it supports our initial hypothesis, but it also  

their genic content.  content. a representative sample of these genes with their enrichment hypergeometric results. We show a representative sample here since every gene has a different $k$ \& $K$  
