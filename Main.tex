...
-barplot(numsabove;mn\&suppl)
-pca(pveincld)
-anntnhgtvsbm

-vschrintrprt
-extbloftppthws (ie I'm not sure if listing examples of the tables is best)
-chkuseof'bs+phn+sbgrp'thrghot
-fgrs:1e-10,a)\&b)thngs
-fgrs:'bs+phn+sbgrp'atall?
...
The specific pathways with significant marginal epistasis signals in the African subgroup share multiple themes across each of the database and phenotype combinations (Supplementary Tables \ref{InterPath-Supp-Table-TopPathways-AllPaths-AllPhenos-Caption}\textcolor{blue}{a-d}). Analyzing height in KEGG, we find multiple pathways representing cellular signaling, immune systems, and heart conditions (Table XX). Analyzing BMI in KEGG, we find similar themes, as well as multiple pathways that involve metabolism. REACTOME pathways show similar themes as well in both phenotypes (Supplementary Table XX, and for all significant pathways see Supplementary Tables \ref{InterPath-Supp-Table-TopPathways-AllPaths-AllPhenos-Caption}\textcolor{blue}{a-d}).  
...
\caption[TBD]{\textbf{Global PCA Plots of the UK BioBank subgroups}. The figure shows the top eight global principal components (PCs) of our UKB subgroup dataset plotted against one another: a) PC1 vs. PC2, b) PC3 vs. PC4, c) PC5 vs. PC6, d) PC7 vs. PC8. PCA was conducted using FlashPCA \citep{Abraham2017} and a post-QC, pruned SNP set. Here `Global' PCs refers to PCs calculated from running PCA on the full dataset of all subgroups together jointly. This is in contrast to `local' PCs, which are calculated by running PCA on each subgroup separately. As a result of this difference, global PCs were created similar to the descriptions in `Population subgroups' of Methods and `Population Subgroup Quality Control' of Supplementary Note, but in lieu of running FlashPCA on each subgroup separately, FlashPCA was run on the full dataset together jointly. Global PCs were only used for these plots, and local PCs were used for quality control and as covariates. In parentheses next to each PC is the percent variance explained (PVE) by each PC in the underlying covariance matrix.}
\label{InterPath-Supp-Figure-UKB-subgroups-PCAPlot}
\end{figure}
\clearpage

Me: implement figure changes: sramachandran: This is the first supplementary figure cited but it's denoted as \#12. Would be good to get order right. Also I'd like to see proportion of variable explained shown in the axes (e.g., PC1 (XX\%)) and then described in the caption that the percentage of variance explained is denote
